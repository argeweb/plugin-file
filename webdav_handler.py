...
import logging
import os
from models.webdav_model import FileModel as Resource, FileResourceData
...
class WebDAVHandler(webapp2.RequestHandler):
    def initialize(self, request, response):
        ...
        if self.request_path.startswith("/assets/files/") >= 0:
            self.request_path = self.request_path.replace("/assets/files/", "")
    ...
    def put(self):
        ...
        data = FileResourceData(blob=self.request.body)
        data.put()
